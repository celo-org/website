...
(async () => {
    ...
    server.get("/papers/whitepaper", (_, res) => {
        res.redirect("/papers/Celo__A_Multi_Asset_Cryptographic_Protocol_for_Decentralized_Social_Payments.pdf");
    });
    ...
    ["/.well-known/openpgpkey/hu/:userId", "/.well-known/openpgpkey/:host/hu/:userId"].forEach((route) => {
        server.get(route, (req, res) => {
            ...
            res.sendFile(path.join(host, "hu", userId), { root: path.join(process.cwd(), "openpgpkey") }, (err) => {
                if (err) {
                    res.sendStatus(404);
                }
            });
    ...
    ["/.well-known/openpgpkey/policy", "/.well-known/openpgpkey/:host/policy"].forEach((route) => {
        server.get(route, (req, res) => {
            ...
            res.sendFile(path.join(host, "policy"), { root: path.join(process.cwd(), "openpgpkey") }, (err) => {
                if (err) {
                    res.sendStatus(404);
                }
            });
    ...
    server.post("/ecosystem/:table", rateLimit, async (req, res) => {
        try {
            await ecoFundSubmission(req.body, req.params.table);
