...
export default function AttestationsTable(props) {
    ...
    <tbody css={borderStyle}>
      ...
      {props.rows?.map(row => {
            return (<TRow loading={props.loading} showProgress={props.showProgress} isMobile={isMobile} key={row.address} max={row.max} name={row.name} address={row.address} count={row.count} twitter={row.twitter} github={row.github} keybase={row.keybase}/>);
        })}
    </tbody>
  </table>
  </div>;
}
const TRow = React.memo((props) => {
    return <tr>
    <td>
...
function firstAndLast4(address) {
...
//# sourceMappingURL=AttestationsTable.jsx.map