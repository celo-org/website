...
function useDropDown() {
    ...
    const setValue = React.useCallback((val) => _setValue(Number(val)), [_setValue]);
    return [value, clear, setValue];
}
function useDropDownOptions(roundsInPhase, phase, round, onSelectRound, onClearRound) {
    const { t } = useTranslation("plumo");
    const dropDownListOptions = React.useMemo(() => roundsInPhase.map((_, index) => {
        return {
            phase,
            id: index.toString(),
...
const Phases = React.memo(function Phases({ phase, setPhase, phase2Started }) {
    const { t } = useTranslation('plumo');
    return <>
  ...
  <span css={radioOne}>
    <Radio selected={phase === 1} labelColor={colors.white} colorWhenSelected={colors.primary} onValueSelected={setPhase} label={t("phase1")} value={1}/>
  </span>
  <span css={radioTwo}>
    <Radio selected={phase === 2} labelColor={colors.white} disabled={!phase2Started} colorWhenSelected={colors.primary} onValueSelected={setPhase} label={t("phase2")} value={2}/>
  </span>
  </>;
});
const rootCss = css({
...
//# sourceMappingURL=Rounds.jsx.map