...
export default function Team(props) {
    ...
    return (<View nativeID="contributors">
      ...
      <GridRow>
        <Cell span={Spans.full} tabletSpan={Spans.full} style={standardStyles.centered}>
          <View style={[
            ...
            {props.contributors.map((person) => (<React.Fragment key={person.name}>
                <Portrait name={person.name} url={person.url} team={person.team} company={person.company} purpose={person.purpose} preview={{ uri: person.preview }} source={{ uri: person.photo }}/>
              </React.Fragment>))}
          </View>
        </Cell>
      </GridRow>
...
const Portrait = React.memo(function _Portrait({ source, name, team, company, preview, purpose, url, }) {
    return (<>
      <Responsive medium={styles.mediumPerson} large={styles.largePerson}>
        <View style={styles.person}>
          ...
          <View style={styles.realImageContainer}>
            <LazyLoadFadin>
              {(onLoad) => (<AspectRatio ratio={1}>
                  <Image accessibilityLabel={`Photo of ${name}`} source={source} onLoad={onLoad} style={styles.photo}/>
                </AspectRatio>)}
            </LazyLoadFadin>
          </View>
