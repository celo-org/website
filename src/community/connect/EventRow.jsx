...
class EventRow extends React.PureComponent {
    ...
    render() {
        const { name, startDate, location, link, endDate, section, description, screen, recap, } = this.props;
            ...
            <TouchableOpacity style={styles.title}>
              ...
              {filteredCoins.map(({ color }, index) => {
                ...
                return (<React.Fragment key={color}>
                    ...
                    {isMobile && index === filteredCoins.length - 1 && (<View style={[styles.chevron, { transform }]}>
                        <Chevron color={colors.dark} opacity={1} size={9}/>
                      </View>)}
                  </React.Fragment>);
            })}
              {isMobile && filteredCoins.length === 0 && (<Chevron color={colors.dark} opacity={1} size={9}/>)}
            </TouchableOpacity>
          </a>
          {isHighlightEvent && (<Text style={[fonts.p, standardStyles.elementalMarginBottom]}>{description}</Text>)}
        </View>
        <View style={!isMobile && styles.row}>
          <Text style={fonts.p}>
            {location} — {printDuration(parseDate(startDate), parseDate(endDate))}
          </Text>
          <EventLink link={link} recap={recap} isMobile={isMobile} isPastEvent={isPastEvent} t={this.props.t}/>
        </View>
      </View>);
    }
}
class EventLink extends React.PureComponent {
    render() {
        const { link, recap, isPastEvent, t, isMobile } = this.props;
        if (link && !isPastEvent && !isMobile) {
            return (<Button size={SIZE.normal} text={t("events.eventPage")} kind={BTN.NAKED} href={link} target="_external"/>);
        }
        if (recap && recap.length) {
            return (<Button size={SIZE.normal} text={t("events.recap")} kind={BTN.NAKED} href={recap} target="_external"/>);
        }
        return null;
    }
}
...
//# sourceMappingURL=EventRow.jsx.map