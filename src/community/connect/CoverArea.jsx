...
export default React.memo(function CoverArea() {
    ...
    return (<GridRow allStyle={[standardStyles.centered, styles.fullScreen]} desktopStyle={styles.root} tabletStyle={styles.root} mobileStyle={styles.mobileRoot}>
      <Cell span={Spans.full} style={[styles.content, !isMobile && styles.contentDesktop]}>
        ...
        <View style={[standardStyles.centered, styles.fadeIn, styles.ctaArea]}>
          <Button style={standardStyles.elementalMarginTop} kind={BTN.PRIMARY} size={SIZE.big} text={t("cover.joinMovement")} href={CeloLinks.discord} iconRight={<Discord color={colors.white} size={24}/>} target={"_blank"}/>
        </View>
      </Cell>
    </GridRow>);
});
const FourWords = React.memo(function _FourWords() {
    ...
    return (<View style={[standardStyles.centered, getWordContainerStyle(screen)]}>
...
function getWordContainerStyle(screen) {
    switch (screen) {
...
//# sourceMappingURL=CoverArea.jsx.map