...
class Tenets extends React.PureComponent {
    componentDidMount() {
        setTimeout(() => window.dispatchEvent(new Event("resize")), 1);
    }
    render() {
        const { t } = this.props;
        return (<View nativeID={hashNav.connect.tenets} style={standardStyles.sectionMarginBottom}>
        <GridRow allStyle={standardStyles.centered} mobileStyle={standardStyles.sectionMarginMobile} tabletStyle={standardStyles.sectionMarginTablet} desktopStyle={standardStyles.sectionMargin}>
        ...
        <Fade distance={"20px"}>
          <Carousel heightMode={"current"} autoplay={false} dragging={true} swiping={true} renderCenterLeftControls={null} renderCenterRightControls={null} renderBottomCenterControls={PagingDots}>
            {TENET_ILLOS.map((image, index) => {
                ...
                return (<Tenet key={number} title={t(`tenets.${number}`)} number={number} copy={t(`tenets.text.${number}`)} headline={t(`tenets.subtitles.${number}`)} imageKey={image}/>);
            })}
          </Carousel>
        </Fade>
      </View>);
...
class Tenet extends React.PureComponent {
    render() {
        const { number, title, headline, imageKey, copy } = this.props;
        return (<View style={styles.tenet}>
        <GridRow tabletStyle={styles.tabletStyle}>
          <Cell span={Spans.half} style={styles.innerPadding}>
            ...
            <ResponsiveImage ratio={1015 / 650} sources={IMAGE_MAP[imageKey]} resizeMode="contain"/>
          </Cell>
...
//# sourceMappingURL=Tenets.jsx.map