...
describe("EcoFund", () => {
    describe("on first render", () => {
        it("is all clear", async () => {
            const { queryAllByText } = render(<TestProvider>
          <EcoFund />
        </TestProvider>);
            queryAllByText("common:validationErrors:email").forEach((element) => expect(element).not.toBeVisible());
            queryAllByText("common:validationErrors:generic").forEach((element) => expect(element).not.toBeVisible());
        });
    });
    describe("when visitor tries submiting without filling out form", () => {
        it("shows validation errors", async () => {
            const { getByText, queryAllByText, queryByText } = render(<TestProvider>
          <EcoFund />
        </TestProvider>);
            const submitButton = getByText("Apply");
            fireEvent.click(submitButton);
            queryAllByText("common:validationErrors:email").forEach((element) => expect(element).toBeVisible());
            queryAllByText("common:validationErrors:generic").forEach((element) => expect(element).toBeVisible());
    ...
    describe("when visitor presses submit after filling out the form", () => {
        function submitSuccess() {
            const options = render(<TestProvider>
          <EcoFund />
        </TestProvider>);
...
//# sourceMappingURL=EcoFund.test.jsx.map