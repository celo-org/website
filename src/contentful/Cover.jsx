...
export default function Cover(props) {
    ...
    return (<GridRow columns={2} darkMode={props.darkMode} wrapperCss={wrapperCss} css={props.illoFirst ? imageFirstRootCss : rootCss}>
      <div css={contentCss}>
        {props.title && (<h1 css={css(props.superSize ? titleCss : rH1, centerMobileCss, props.darkMode && whiteText)}>
          {props.title}
        </h1>)}
      ...
      <div css={props.imageFit === "contain"
            ? css(illoContain, {
                [WHEN_TABLET]: {
        ...
        <picture>
          {resolution === 2 && (<>
              <source media={`(min-width: ${TABLET_BREAKPOINT}px)`} srcSet={`${props.imageDesktop?.fields.file.url} 2x`}/>
              <source media={`(max-width: ${TABLET_BREAKPOINT}px)`} srcSet={`${props.imageMobile?.fields.file.url} 2x`}/>
            </>)}
          <source media={`(min-width: ${TABLET_BREAKPOINT}px)`} srcSet={trueWidth(props.imageDesktop?.fields.file, resolution)}/>
          <img width={viewSize(size?.width, resolution)} height={viewSize(size?.height, resolution)} css={css(props.illoFirst ? imageFirstCss : imageCss, props.verticalPosition === "flushBottomText" && flushBottomCss, props.imageFit === "contain" && imageContain)} src={trueWidth(props.imageMobile?.fields.file, resolution)} alt={props.imageDesktop?.fields.description}/>
        </picture>
      </div>
    </GridRow>);
}
function viewSize(number, res) {
    return Math.round(number / res);
}
function trueWidth(file, res) {
    if (res === 1) {
        return file?.url;
    }
    else {
