...
export default function Form(props) {
    ...
    return (<form action={props.route} method="post" css={styles} onSubmit={handleSubmit(onSubmit)}>
      {props.fields.map((input) => {
            ...
            {input.fields.type === "multiline" ? (<textarea onFocus={onFocus} aria-invalid={ariaInvalid} name={input.fields.name} css={inputDarkStyle} rows={5} {...attributes}/>) : (<input name={input.fields.name} type={input.fields.type} css={inputDarkStyle} {...attributes}/>)}
            <MessageDisplay css={errorStyle} isShowing={formState.errors[input.fields.name]?.type === "required"}>
              {t("validationErrors.fieldIsRequired", { field: input.fields.label })}
            </MessageDisplay>
            <MessageDisplay css={errorStyle} isShowing={formState.errors[input.fields.name]?.type === "email"}>
              {t("validationErrors.email")}
            </MessageDisplay>
            <MessageDisplay css={errorStyle} isShowing={formState.errors[input.fields.name]?.type === "url"}>
