...
import { flex, flexRow, fonts, WHEN_MOBILE, WHEN_TABLET, whiteText, standardStyles, WHEN_DESKTOP, } from "src/estyles";
const MENU = [
    menu.HOME,
    menu.ABOUT_US,
    menu.JOBS,
...
export default function Footer({ hideForm, darkMode }) {
    ...
    return (<>
      {!hideForm && (<GridRow columns={1} darkMode={darkMode} css={css(standardStyles.centered, {
          ...
          <div css={formStyle}>
            <img src={Evelope.src} css={emailLogoCss} width={50} height={60} alt={t("footer.emailIconAlt")}/>
            <p css={receiveUpdatesCss}>{t("receiveUpdates")}</p>
            <EmailForm submitText={t("signUp")} route={"contacts"} isDarkMode={false}/>
          </div>
        </GridRow>)}
      <GridRow columns={3} darkMode={darkMode} css={css({
        ...
        <Cell span={Spans.two} tabletSpan={Spans.three}>
          {isMobile ? (<MobileLinks footerMenu={footerMenu} darkMode={darkMode}/>) : (<div css={linksAreaCss}>
              <FooterColumn css={linkColumnStartCss} heading={"Celo"} links={footerMenu} darkMode={darkMode}/>
              ...
              <FooterColumn darkMode={darkMode} css={linkColumnEndCss} heading={t("footer.social")} links={socialMenu(darkMode)}/>
            </div>)}
        </Cell>
      </GridRow>
      <GridRow columns={1} darkMode={darkMode} css={css({
...
function MobileLinks({ footerMenu, darkMode }) {
    ...
    return (<>
      <div css={flexRow}>
        ...
        <FooterColumn darkMode={darkMode} heading={t("footer.social")} links={socialMenu(darkMode)} css={endMobileColumnCss}/>
      </div>
      <div css={flexRow}>
        <FooterColumn darkMode={darkMode} heading={t("footer.resources")} links={RESOURCE_MENU}/>
        <FooterColumn darkMode={darkMode} heading={t("footer.technology")} links={TECH_MENU} css={endMobileColumnCss}/>
      </div>
    </>);
}
const Details = React.memo(function _Details({ darkMode }) {
    const { t } = useTranslation(NameSpaces.common);
    const fontStyling = css(fonts.legal, detailsTextCss, darkMode && whiteText);
    return (<div css={detailsCss}>
