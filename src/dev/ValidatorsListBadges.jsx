...
class ValidatorsListBadges extends React.PureComponent {
    render() {
        const { address } = this.props;
        const badges = allBadges.filter(({ list }) => list.has(address.toLowerCase()));
        return (<View style={styles.container}>
        {badges.map(({ src, title }) => (<img key={src} src={src} title={title}/>))}
      </View>);
    }
}
...
//# sourceMappingURL=ValidatorsListBadges.jsx.map