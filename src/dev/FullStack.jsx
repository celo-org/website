...
const LayersIllo = dynamic(import("src/dev/LayersIllo"));
var Levels;
(function (Levels) {
    Levels[Levels["apps"] = 0] = "apps";
    Levels[Levels["contracts"] = 1] = "contracts";
    Levels[Levels["blockchains"] = 2] = "blockchains";
    Levels[Levels["code"] = 3] = "code";
})(Levels || (Levels = {}));
var StickyMode;
(function (StickyMode) {
    StickyMode[StickyMode["normal"] = 0] = "normal";
    StickyMode[StickyMode["attachToBottom"] = 1] = "attachToBottom";
    StickyMode[StickyMode["fixed"] = 2] = "fixed";
})(StickyMode || (StickyMode = {}));
const GLASS_CEILING = 160;
class FullStack extends React.PureComponent {
    state = { selection: Levels.apps, mode: StickyMode.normal };
    ref = React.createRef();
    illoRef = React.createRef();
    handleScroll = throttle(() => {
        if (!(this.props.screen === ScreenSizes.DESKTOP)) {
            this.setState({ mode: StickyMode.normal });
            return;
        }
        const element = findNodeHandle(this.ref.current);
        if (!element) {
            return;
        }
    ...
    autoSetHighlight(clientRect, illoHeight) {
    ...
    render() {
        ...
        return (<View style={standardStyles.darkBackground} ref={this.ref}>
        <GridRow desktopStyle={standardStyles.sectionMarginTop} tabletStyle={[standardStyles.sectionMarginTopTablet, styles.tabletContainer]} mobileStyle={standardStyles.sectionMarginTopMobile} allStyle={styles.container}>
          <Cell span={Spans.half} tabletSpan={Spans.full}>
            <View style={this.modeStyle()} ref={this.illoRef}>
              <View style={styles.illoContainer}>
                ...
                {isDesktop && (<LayersIllo activeLayer={activeLayer} onSelectLayer={this.setLevel}/>)}
              </View>
            </View>
          </Cell>
          <Cell span={Spans.half} tabletSpan={Spans.full} style={isDesktop && styles.stackContainer}>
            <StackSection onPress={this.setL1} id={hashNav.build.applications} isSelected={this.state.selection === Levels.apps || !isDesktop} title={t("mobile.title")} text={t("mobile.text")} buttonOne={{ title: t("installWallet"), href: CeloLinks.walletApp }} buttonTwo={{ title: t("seeCode"), href: CeloLinks.gitHub }} buttonMiddle={{ title: t("getFunds"), href: CeloLinks.faucet }}>
            ...
            <StackSection onPress={this.setL2} id={hashNav.build.contracts} isSelected={this.state.selection === Levels.contracts || !isDesktop} title={t("protocol.title")} text={t("protocol.text")} buttonOne={{ title: t("readMore"), href: CeloLinks.docsOverview }} buttonTwo={{ title: t("seeCode"), href: CeloLinks.gitHub }}>
            ...
            <StackSection onPress={this.setL3} id={hashNav.build.blockchain} isSelected={this.state.selection === Levels.blockchains || !isDesktop} title={t("proof.title")} text={t("proof.text")} buttonOne={{ title: t("readMore"), href: CeloLinks.docsOverview }} buttonTwo={{ title: t("seeCode"), href: CeloLinks.blockChainRepo }}>
            ...
            <View style={[
              ...
              <Button disabled={isBrowseCodeFaded} kind={BTN.PRIMARY} text={t("stackBrowseButton")} target="_blank" href={CeloLinks.gitHub} iconRight={<Octocat color={colors.white} size={20}/>}/>
            </View>
          </Cell>
        </GridRow>
      </View>);
...
//# sourceMappingURL=FullStack.jsx.map