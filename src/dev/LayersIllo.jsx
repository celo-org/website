...
export default React.memo(function LayersIllo({ activeLayer, onSelectLayer }) {
    return (<svg width="100%" height="290px" viewBox="0 0 367 147" fill="none">
      {VECTORS.map((vector, index) => {
            const onPress = () => onSelectLayer(index);
            return (<Path key={vector} d={vector} onPress={onPress} style={[
                    styles.clicky,
                    activeLayer === "all" || activeLayer === index ? styles.active : styles.inactive,
                ]} stroke={colors.white} fill={"transparent"}/>);
        })}
    </svg>);
});
...
//# sourceMappingURL=LayersIllo.jsx.map