...
import { isPinned, togglePin } from "src/utils/validators";
const unknownGroupName = "Unnamed Group";
const unknownValidatorName = "Unnamed Validator";
class ValidatorsListRow extends React.PureComponent {
    state = {
        tooltip: false,
    };
    ...
    constructor(...args) {
        ...
        this.removeDocumentListener = () => document.removeEventListener("click", onDocumentClick, false);
    }
    ...
    render() {
        ...
        return (<div style={tooltip ? { zIndex: 2 } : {}}>
        <View style={[styles.tableRow, styles.tableRowCont, tooltip ? { zIndex: 3 } : {}]}>
          ...
          <View style={[styles.tableCell, styles.tableCellTitle]}>
            <Text style={[
              ...
              <Chevron direction={Direction.right} opacity={expanded ? 1 : 0.4} color={colors.white} size={10}/>
            </Text>
            <Text style={[styles.defaultText, styles.tableCellTitleRows]}>
              <Text style={[styles.defaultText, styles.tableCellTitleFirstRowWrapper]}>
                <Text style={[styles.defaultText, styles.tableCellTitleFirstRow]} numberOfLines={1} ellipsizeMode="tail" accessibilityRole="link" href={`https://explorer.celo.org/address/${group.address}/celo`} hrefAttrs={hrefAttrs}>
                  {group.name || unknownGroupName}
                </Text>

                {!!group.claims.length && (<Text style={[styles.defaultText, styles.checkmark]}>
                    <div ref={this.tooltipRef} onClick={this.toggleTooltip}>
                      <Checkmark color={colors.black} size={8}/>
                    </div>

                    {tooltip && (<Text style={[styles.defaultText, styles.tooltip]}>
                        {group.claims.map((domain, i) => (<Text key={domain} style={[styles.defaultText, styles.tooltipRow]}>
                            {i + 1}.
                            <Text accessibilityRole="link" hrefAttrs={hrefAttrs} href={`https://${domain}`} style={[styles.defaultText, styles.tooltipText]}>
                              {domain}
                            </Text>
          ...
          <Text style={[styles.defaultText, styles.tableCell, styles.tableCellCenter, styles.sizeM]} numberOfLines={1} ellipsizeMode="tail">
          ...
          <Text style={[styles.defaultText, styles.tableCell, styles.sizeXL, styles.tableCellBars]}>
            ...
            <Text style={[styles.defaultText, styles.tableCellBarsRows]}>
              ...
              <ProgressCutBar bars={Math.min(Math.floor(+group.receivableVotes * 2) || 1, 6)} progress={Math.floor(+group.votes)}/>
            </Text>
          </Text>
          <Text style={[styles.defaultText, styles.tableCell, styles.tableCellCenter, styles.sizeM]} numberOfLines={2} ellipsizeMode="tail">
          ...
          <Text style={[styles.defaultText, styles.tableCell, styles.tableCellCenter, styles.sizeS]} numberOfLines={1} ellipsizeMode="tail">
            {formatNumber(group.attestation, 1)}%
          </Text>
        </View>
        {expanded && (<>
            {group.validators.map((validator, j) => (<View key={`${group.id}.${j}`} style={[styles.pStatic, styles.tableRow]}>
                <Text style={[
                        styles.defaultText,
                        styles.tableCell,
                  ...
                  <Text style={[styles.defaultText, styles.tableCellTitleRows]}>
                    <Text style={[
                        ...
                        styles.tableSecondaryCell,
                    ]} accessibilityRole="link" href={`https://explorer.celo.org/address/${validator.address}/celo`} hrefAttrs={hrefAttrs}>
                ...
                <Text style={[styles.tableSecondaryCell, styles.sizeM]} numberOfLines={1} ellipsizeMode="tail">
                  <View style={[styles.circle, styles[validator.elected ? "circleOk" : "circleError"]]}/>
                </Text>
...
//# sourceMappingURL=ValidatorsListRow.jsx.map