...
export default function TopBar({ current, kitName }) {
    ...
    return (<View style={standardStyles.centered}>
      <View style={[standardStyles.row, styles.container, isMobile && styles.containerMobile]}>
        <View style={styles.rowVerticalCenter}>
          ...
          <a href={current} css={kitBrand}>
            <TouchableOpacity style={styles.rowVerticalCenter}>
              <Text style={[fonts.h3, styles.title]}>
                {kitName}
              </Text>
            </TouchableOpacity>
          </a>
        ...
        <View style={styles.rowVerticalCenter}>
          {isMobile ? (<Hamburger onPress={toggleKits} isOpen={showingKits} color={colors.dark} style={styles.hamburger}/>) : (<View style={styles.kits}>
              <Kits current={current}/>
            </View>)}
          {showingKits && (<View style={styles.kitsMobileShown}>
...
const Kits = React.memo(({ current }) => {
    const { isTablet } = useScreenSize();
    const displayedKits = isTablet ? KITS.filter((kit) => kit.link !== current) : KITS;
    return (<>
      {displayedKits.map((kit) => {
            return (<Navigation key={kit.link} style={styles.navLink} text={kit.name} link={kit.link} selected={kit.link === current} theme={NavigationTheme.LIGHT}/>);
        })}
    </>);
});
