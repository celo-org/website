...
export default React.memo(function Search({ value, onChange }) {
    const { t } = useTranslation(NameSpaces.brand);
    return (<View style={[brandStyles.gap, styles.root]}>
      <Text style={[fonts.h5, standardStyles.blockMarginTop, standardStyles.elementalMarginBottom]}>
        {t("icons.searchLabel")}
      </Text>
      <TextInput name={"search"} style={[standardStyles.input, fonts.p]} placeholder={t("icons.searchPlaceholder")} accessibilityLabel={"search"} value={value} onChange={onChange} focusStyle={standardStyles.inputFocused}/>
    </View>);
});
