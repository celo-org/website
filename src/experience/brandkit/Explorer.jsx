...
export function Explorer({ icons }) {
    ...
    return (<View style={styles.root}>
      ...
      <View style={brandStyles.tiling}>
        {icons.map((icon) => (<View key={icon.id} testID={icon.id} style={!visibleIcons.has(icon.id) && styles.offScreen}>
            <IconShowcase key={icon.name} ratio={1} description={icon.description} name={icon.name} preview={icon.preview} uri={icon.uri} loading={false} assetType={AssetTypes.icon} size={160}/>
          </View>))}
      </View>
    </View>);
}
...
function useVisibleIconIDs(query, initial) {
