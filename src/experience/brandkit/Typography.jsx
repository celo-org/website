...
export default React.memo(withNamespaces(NameSpaces.brand)(function Typography({ t }) {
    return (<Page title={t("typography.title")} metaDescription={t("typography.headline")} path={TYPE_PATH} sections={[
            { id: brandTypography.overview, children: <Overview /> },
            { id: brandTypography.scale, children: <TypeScale /> },
        ]}/>);
}));
const Overview = withNamespaces(NameSpaces.brand)(React.memo(function _Overview({ t }) {
    return (<View>
        <PageHeadline title={t("typography.title")} headline={t("typography.headline")} style={standardStyles.blockMarginBottom}/>
        <View style={brandStyles.gap}>
          ...
          <DownloadButton uri="https://fonts.google.com/specimen/EB+Garamond" trackingData={GARMOND_TRACKING}/>
          <View style={standardStyles.blockMarginTopTablet}>
            <H3 style={styles.hero}>{t("typography.subFontTitle")}</H3>
            <Text style={[fonts.p, standardStyles.elementalMarginBottom]}>
              {t("typography.subFontText")}
            </Text>
            <DownloadButton uri="https://indestructibletype.com/Jost.html" trackingData={JOST_TRACKING}/>
          </View>
        </View>
      </View>);
}));
...
const TypeScale = withNamespaces(NameSpaces.brand)(React.memo(withScreenSize(function _TypeScale({ t, screen }) {
    return (<View>
          ...
          <View style={[
            ...
            return (<View key={typeface.name} style={[
