...
const KeyImageryWrapped = React.memo(function KeyImagery({ illos, graphics, }) {
    const { t } = useTranslation(NameSpaces.brand);
    return (<Page title={t("keyImagery.title")} metaDescription={t("keyImagery.headline")} path={IMAGERY_PATH} sections={[
            {
...
const Illustrations = React.memo(function _Illustrations({ data }) {
    const size = useIlloSize();
    const { t } = useTranslation(NameSpaces.brand);
    return (<View style={standardStyles.blockMarginTopTablet}>
      ...
      <View style={[brandStyles.tiling, styles.illustrationsArea]}>
        {data &&
            data.map((illo) => (<Showcase key={illo.id} ratio={1.3} assetType={AssetTypes.illustration} description={illo.description} name={illo.name} preview={illo.preview} uri={illo.uri} loading={false} size={size}/>))}
      </View>
    </View>);
});
const AbstractGraphics = React.memo(function _AbstractGraphics({ data }) {
    const { t } = useTranslation(NameSpaces.brand);
    return (<View style={standardStyles.sectionMarginTop}>
      <H2 style={[brandStyles.gap, standardStyles.elementalMarginBottom]}>
      ...
      <View style={brandStyles.tiling}>
        {data &&
            data.map((graphic) => (<Showcase key={graphic.id} ratio={344 / 172} assetType={AssetTypes.graphic} description={graphic.description} name={graphic.name} preview={graphic.preview} uri={graphic.uri} loading={false} size={"100%"}/>))}
      </View>
    </View>);
});
...
//# sourceMappingURL=KeyImagery.jsx.map