...
export default React.memo(function Intro() {
    ...
    return (<>
      <Page title="Composition" path={COMPOSITION_PATH} metaDescription={t("composition.introduction")} sections={[
            { id: hashNav.brandComposition.overview, children: <Overview /> },
            { id: hashNav.brandComposition.grid, children: <GridArea /> },
        ]}/>
    </>);
});
const imageGridArea = { gridArea: "image" };
function Overview() {
    const { t } = useTranslation(NameSpaces.brand);
    return (<>
      <PageHeadline title={t("composition.title")} headline={t("composition.headline")} style={standardStyles.blockMarginBottom}/>
...
function GridExamples({ size, colStart, colEnd, row }) {
    return (<View style={[styles.gridExample, { gridArea: `${row} / ${colStart} / ${row} / ${colEnd} ` }]}>
      <Text style={[fonts.a, styles.gridExampleWithin]}>{size}</Text>
    </View>);
}
function AlignmentExampleTitle() {
    const { t } = useTranslation(NameSpaces.brand);
    return (<H4 style={[standardStyles.elementalMarginBottom, textStyles.center]}>
      {t("composition.alignmentExampleTitle")}
    </H4>);
}
...
//# sourceMappingURL=Composition.jsx.map