...
import { ACCENT_PALETTE, BACKGROUND_PALETTE, GRAY_PALETTE, PRIMARY_PALETTE, } from "src/experience/common/data";
...
export default React.memo(withNamespaces(NameSpaces.brand)(function Color({ t }) {
    return (<Page title={t("color.title")} metaDescription={t("colo.headline")} path={COLOR_PATH} sections={[
            { id: brandColor.overview, children: <Overview /> },
            { id: brandColor.backgrounds, children: <Backgrounds /> },
        ]}/>);
}));
const Overview = withNamespaces(NameSpaces.brand)(function _Overview({ t }) {
    return (<View>
      <PageHeadline title={t("color.title")} style={standardStyles.blockMarginBottom}/>
      <Palette title={t("color.primaries")} text={t("color.primariesText")} colors={PRIMARY_PALETTE}/>
...
const Backgrounds = withNamespaces(NameSpaces.brand)(withScreenSize(function _Backgrounds({ t, screen }) {
    ...
    return (<View>
        ...
        <View style={screen === ScreenSizes.DESKTOP ? brandStyles.tiling : { width: "100%" }}>
          ...
          <View style={stylesForJudgemnt}>
            <Judgement is={Value.Bad}>
              <Lorem color={colors.white} backgroundColor={colors.gray} image={require("src/experience/brandkit/images/lilah.jpg")}/>
            </Judgement>
            <Judgement is={Value.Good}>
              <Lorem color={colors.white} backgroundColor={colors.dark} image={require("src/experience/brandkit/images/lilahOverlay.jpg")}/>
            </Judgement>
          </View>
          <View style={stylesForJudgemnt}>
            <Judgement is={Value.Bad}>
...
function Lorem({ backgroundColor, color, withGap, hasBorder, image }) {
    const Component = image ? ImageBackground : View;
    return (<Component source={image} style={[
