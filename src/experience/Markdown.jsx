...
import { DesignForAll, EmbodyHumility, InnovatingOnMoney, StrivingForBeauty, } from "src/experience/eventkit/Tenents";
...
function SmartAnchor({ children, href }) {
    ...
    return (<InlineAnchor target={target} href={href}>
      {children}
    </InlineAnchor>);
}
function PrimeButton({ children, href, external, }) {
    const track = React.useCallback(() => trackOpen({ name: children, type: Types.Action }), [children]);
    return (<Button style={standardStyles.halfElement} kind={BTN.PRIMARY} text={children} href={href} onPress={track} target={external ? "_blank" : ""}/>);
}
...
const OPTIONS = {
    overrides: {
        ...
        h3: ({ children }) => (<View style={standardStyles.blockMarginTopTablet}>
        <H3>{children}</H3>
      </View>),
        h4: {
            component: H4,
