...
const emailErrorStyle = (errors) => {
    if (errors.includes("email")) {
        return { borderColor: colors.error };
    }
    return {};
};
export default React.memo(function EmailForm({ isDarkMode, submitText, placeholder, listID = NEWSLETTER_LIST, route = "/contacts", }) {
    const inputTheme = isDarkMode ? styles.inputDarkMode : styles.inputLightMode;
    const { isDesktop } = useScreenSize();
    const { t } = useTranslation(NameSpaces.common);
    return (<Form route={route} blankForm={{ ...blankForm, list: listID }} validateWith={validateFields}>
      {({ formState, onInput, onSubmit }) => {
            ...
            return (<Responsive large={styles.container}>
            <View style={styles.mobileContainer}>
              <Responsive large={[fonts.p, styles.input, inputTheme, styles.inputDesktop, borderStyle]}>
                <TextInput style={[fonts.p, styles.input, inputTheme, borderStyle]} focusStyle={isDarkMode ? standardStyles.inputDarkFocused : standardStyles.inputFocused} onChangeText={onChange} placeholder={placeholder || t("common:email") + "*"} placeholderTextColor={isDarkMode ? colors.placeholderDarkMode : colors.placeholderGray} name="email" type="email" value={formState.form.email} required={true}/>
              </Responsive>
              {!isDesktop && (<View style={!!formState.errors.length && styles.feedbackMobile}>
                  <ErrorDisplay isShowing={hasError} field={errorKey}/>
                </View>)}
              <Responsive large={[styles.submitBtn, styles.submitBtnDesktop]}>
                <SubmitButton isLoading={formState.isLoading} onPress={onSubmit} text={submitText} size={SIZE.fullWidth}/>
              </Responsive>
...
//# sourceMappingURL=EmailForm.jsx.map