...
export function BlueBanner(props) {
    ...
    return (<View testID={"banner"} ref={ref} style={[styles.container, props.isVisible && styles.isVisible]}>
      <View style={styles.insideContainer}>
        <Text accessibilityRole="link" hrefAttrs={hrefAttrs} href={props.link} style={[fonts.navigation, textStyles.medium, styles.text]}>
          {props.children}
...
export default function Announcement(props) {
    const state = useAsync(() => getAnnouncement(props.onVisibilityChange), [props.onVisibilityChange]);
    const { setBannerHeight } = useScreenSize();
    if (state.status === "success") {
        return (<BlueBanner isVisible={state.result.live} link={state.result.link} getRealHeight={setBannerHeight}>
        {state.result.text}
      </BlueBanner>);
    }
    return null;
}
//# sourceMappingURL=BlueBanner.jsx.map