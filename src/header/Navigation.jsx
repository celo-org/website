...
const PATH_TO_ATTRIBUTES = Object.keys(pagePaths).reduce((last, key) => {
...
function useAttributes() {
    ...
    return (PATH_TO_ATTRIBUTES[pathname] || {
        isDark: false,
        translucent: null,
        menuHidePoint: null,
...
export default function Header(props) {
    ...
    const isDarkMode = attributes.isDark || props.darkMode || (attributes.translucent && !belowFoldUpScroll);
    ...
    return (<div css={css(styles.container, { top: isHomePage && isBannerShowing ? bannerHeight : 0 }, menuFaded && { height: 0 }, mobileMenuActive && styles.mobileMenuActive)}>
      {isHomePage && <BlueBanner onVisibilityChange={setBannerVisible}/>}

      <div css={css(styles.menuContainer, styles.background, backgroundColor, styles.fadeTransition, menuFaded ? styles.menuInvisible : styles.menuVisible)}>
      ...
      {mobileMenuActive && (<div css={styles.menuActive}>
          <div css={styles.mobileOpenContainer}>
            <MobileMenu currentPage={pathname} menu={mobileMenu}/>
          </div>
        </div>)}
      <MobileMenuIcon isDarkMode={isDarkMode} willShowHamburger={willShowHamburger} isHomePage={isHomePage} mobileMenuActive={mobileMenuActive} bannerHeight={isBannerShowing ? bannerHeight : 0} clickHamburger={clickHamburger}/>
    </div>);
}
const HomeLogo = React.memo(function _HomeLogo({ menuFaded, isDarkMode, allWhiteLogo, }) {
    return (<Link href={"/"}>
      <div css={styles.logoLeftContainer}>
        <div css={styles.logoContainer}>
          <>
            <div css={css(styles.fadeTransition, menuFaded ? styles.menuInvisible : styles.menuVisible)}>
              {isDarkMode ? (<LogoDarkBg height={30} allWhite={allWhiteLogo}/>) : (<LogoLightBg height={30}/>)}
            </div>
          </>
        </div>
      </div>
...
const NavigationLinks = React.memo(function _NavigationLinks(props) {
    ...
    return (<nav css={[
      ...
      {menuItems.map((item) => (<div key={item.link} css={styles.linkWrapper}>
          <Button kind={props.isDarkMode ? BTN.DARKNAV : BTN.NAV} href={item.link} text={t(item.name)}/>
          {pathname === item.link && (<div css={styles.activeTab}>
              <OvalCoin color={colors.primary} size={10}/>
            </div>)}
        </div>))}
      <div css={styles.linkWrapper}>
        <Button kind={props.isDarkMode ? BTN.DARKNAV : BTN.NAV} href={"https://medium.com/CeloHQ"} text={""} target={"_blank"} iconRight={<MediumLogo height={20} color={foregroundColor} wrapWithLink={false}/>}/>
      </div>
      <div css={[styles.linkWrapper, styles.lastLink]}>
        <Button kind={props.isDarkMode ? BTN.DARKNAV : BTN.NAV} href={CeloLinks.gitHub} text={""} target={"_blank"} iconRight={<Octocat size={22} color={props.isDarkMode ? colors.white : colors.dark}/>}/>
      </div>
    </nav>);
});
function MobileMenuIcon(props) {
    const foregroundColor = props.isDarkMode ? colors.white : colors.dark;
    const containerStyle = css(styles.hamburger, styles.fadeTransition, props.willShowHamburger && styles.hamburgerShowing, props.isHomePage &&
        !props.mobileMenuActive && {
        transform: `translateY(${props.bannerHeight}px)`,
    });
    return (<div css={containerStyle}>
      <Hamburger isOpen={props.mobileMenuActive} onPress={props.clickHamburger} color={foregroundColor}/>
    </div>);
}
function flexCss(cssStyle) {
    return css({
        display: "flex",
    }, cssStyle);
}
