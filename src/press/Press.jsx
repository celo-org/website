      ...
      <GridRow columns={1} css={containercss}>
        <div css={logoContainerCss}>
          {props.list?.map(({ sys, fields }) => {
            ...
            return (<a key={sys.id} href={fields.url} target={"_blank"} rel="noopener">
                <picture>
                  ...
                  <img alt={fields.image?.fields?.description} loading={"lazy"} height={24} width={24 * ratio} css={css(logoCss)} src={file?.url}/>
                </picture>
              </a>);
        })}
        </div>
        <div css={links}>
          <Button text={t("recentNews")} kind={BTN.NAKED} size={SIZE.normal} href={MENU.PRESS.link} align={"center"}/>
        </div>
      </GridRow>
    </div>);
}
const backgroundCss = css(flex, {
    ...
    [WHEN_TABLET_AND_UP]: {
        ...
        background: "linear-gradient(180deg, rgba(46, 51, 56, 0.1) 1%, rgba(46, 51, 56, 0.6) 20%, #2E3338 48.13%)",
        zIndex: 20,
        opacity: 1,
        paddingTop: 60,
        paddingBottom: 60,
