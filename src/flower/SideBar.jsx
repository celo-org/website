      ...
      <View style={[styles.root, isMobile && styles.rootMobile, isOpen ? openStyle : closedStyle]}>
        <View>
          ...
          <View style={standardStyles.elementalMarginTop}>
            <Button style={styles.chevronButton} kind={BTN.DARKNAKED} text="Read about the poem" href="https://medium.com/celoorg/a-celebration-of-heart-for-celo-44bbbba94a2c" size={SIZE.normal}/>
          </View>
...
function CopyButton() {
    ...
    return (<LightButton onPress={copyURL} style={styles.copyButton}>
...
const TweetButton = React.memo(() => {
    ...
    return (<>
      <LightButton href={`https://twitter.com/intent/tweet?hashtags=celoflora&related=celoOrg&via=celoOrg&text=${text}&url=${url}`}>
        <TweetLogo height={16} color={colors.dark}/> Tweet
      </LightButton>
    </>);
});
function Contributor({ role, name }) {
    return (<View style={styles.contributor}>
