...
const HomeBackers = ({ t }) => {
    return (<GridRow mobileStyle={standardStyles.sectionMarginTopMobile} tabletStyle={standardStyles.sectionMarginTopTablet} desktopStyle={standardStyles.sectionMarginTop}>
      <Cell span={Spans.full} style={standardStyles.centered}>
        <Responsive medium={[styles.logoContainerTablet]} large={[styles.logoContainerDesktop]}>
          <View style={styles.logoContainer}>
            {homeBackers.map((backer) => (<Image resizeMode={"contain"} style={[styles.backerLogo, { height: backer.height }]} key={backer.id} source={{ uri: backer.image }}/>))}
          </View>
        </Responsive>
        <View style={[styles.linkContainer, standardStyles.elementalMarginTop]}>
          <Button kind={BTN.NAKED} size={SIZE.normal} href={`${menuItems.ABOUT_US.link}#${hashNav.about.backers}`} text={t("backers")}/>
        </View>
      </Cell>
    </GridRow>);
};
...
//# sourceMappingURL=HomeBackers.jsx.map