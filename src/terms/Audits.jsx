...
class Audits extends React.PureComponent {
    render() {
        const { t } = this.props;
        return (<>
        <OpenGraph title={"Celo | Audits & Analyses"} path={NameSpaces.audits} description={t("metaDescription")}/>
        <View style={styles.container}>
          ...
          {Object.keys(AUDITS).map((type) => {
                return (<SideTitledSection key={type} span={Spans.three4th} title={t(type)}>
                {AUDITS[type].map((audit) => (<View style={styles.reference} key={audit.title}>
                    <Text style={fonts.p}>
